...
<div class="px-[40px] pb-[40px] bg-white" x-data="{ activeTab: '{{ array_key_first($folders) }}' }">
    ...
    <div class="flex flex-wrap border-b-[2px] border-[#B58E03]">
        @foreach ($folders as $key => $label)
            <button
                ...
                class="px-2 py-3 text-sm border-r border-gray-300 focus:outline-none transition w-full md:w-[25%]">
                <span class="text-sm md:text-lg">{{ $label }}</span>
            </button>
        @endforeach
    </div>
    ...
    @foreach ($folders as $key => $label)
        ...
        <div x-show="activeTab === '{{ $key }}'" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6 mt-6">
