<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const recaptchaSiteKey = document.querySelector('meta[name="recaptcha-sitekey"]').getAttribute('content');
...
<div class="bg-gray-100 p-8 shadow-md w-full mx-auto">
    ...
    <form class="space-y-6" action="/contact" method="POST">
        <!-- Input Fields -->
        <input type="hidden" name="_token" id="csrf-token-input" value="" autocomplete="off">
        ...
        <label class="custom-label flex items-start mt-4">
            <div class="bg-[#ccc] shadow w-6 h-6 p-1 flex justify-center items-center mr-2 relative cursor-pointer">
                <input type="checkbox" id="consent" name="consent" class="sr-only" onchange="this.nextElementSibling.classList.toggle('opacity-0')" required>
        ...
        <div class="mt-6">
            <button class="bg-[#143c5a] hover:bg-[#39a7cc] text-white pb-[16px] py-[15px]">
                ...
                <span class="pl-[20px] pr-[20px] pt-[16px] pb-[18px] border border-[#39a7cc] bg-[#39a7cc]">▸</span>
            </button>
        </div>
    </form>
</div>
